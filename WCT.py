...
parser.add_argument('--vgg1', default='models/pth_models/vgg_normalised_conv1_1.pth', help='Path to the VGG conv1_1')
parser.add_argument('--vgg2', default='models/pth_models/vgg_normalised_conv2_1.pth', help='Path to the VGG conv2_1')
parser.add_argument('--vgg3', default='models/pth_models/vgg_normalised_conv3_1.pth', help='Path to the VGG conv3_1')
parser.add_argument('--vgg4', default='models/pth_models/vgg_normalised_conv4_1.pth', help='Path to the VGG conv4_1')
parser.add_argument('--vgg5', default='models/pth_models/vgg_normalised_conv5_1.pth', help='Path to the VGG conv5_1')
parser.add_argument('--decoder5', default='models/pth_models/feature_invertor_conv5_1.pth', help='Path to the decoder5')
parser.add_argument('--decoder4', default='models/pth_models/feature_invertor_conv4_1.pth', help='Path to the decoder4')
parser.add_argument('--decoder3', default='models/pth_models/feature_invertor_conv3_1.pth', help='Path to the decoder3')
parser.add_argument('--decoder2', default='models/pth_models/feature_invertor_conv2_1.pth', help='Path to the decoder2')
parser.add_argument('--decoder1', default='models/pth_models/feature_invertor_conv1_1.pth', help='Path to the decoder1')
...
#     print(f'contentImg.size={contentImg.size()}')
#     imname = imname[0]
...
for c_name in c_name_list:
    for s_name in s_name_list:
        s_basename = os.path.splitext(c_name)[0]
        c_basename = os.path.splitext(s_name)[0]
        c_path = os.path.join(c_dir, c_name)
        s_path = os.path.join(s_dir, s_name)
        print(f'processing [{c_path}] and [{s_path}]')
        contentImg = transforms.ToTensor()(Image.open(c_path)).unsqueeze(0)
        styleImg = transforms.ToTensor()(Image.open(s_path)).unsqueeze(0)
        imname = f'{c_basename}_{s_basename}.bmp'
        if (args.cuda):
            contentImg = contentImg.cuda(args.gpu)
            styleImg = styleImg.cuda(args.gpu)
        cImg = Variable(contentImg, volatile=True)
...
print(f'Processed {len(c_name_list) * len(s_name_list)} images. Averaged time is {avgTime / (len(c_name_list) * len(s_name_list))}')
